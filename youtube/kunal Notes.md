...
```
### [Even Digits](https://leetcode.com/problems/find-numbers-with-even-number-of-digits/)
...
//        System.out.println(findNumbers(nums));  <----- (1)

        System.out.println(digits2(-345678));
    }
    static int findNumbers(int[] nums) {    // [1]
        int count = 0;
        for(int num : nums) {
            if (even(num)) {     //Check whether the number is even or not<----- (2)
                count++;
            }
        }
        return count;
    ...
    static boolean even(int num) { // [2]
        int numberOfDigits = digits(num); //<----- (3)
        /*
